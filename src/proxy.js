...
// console.log("AUTHME RESULT:", isValid);
//         if (isValid) {
//            console.log("AUTH VALID -> allowing request");
//           return NextResponse.next(); // all good, let them through
//         }
//       }
...
export async function proxy(request) {
  ...
  if (
    ...
    // Access token exists
    if (accessToken) {
      const decoded = checkAuth(accessToken.value);

      if (decoded) {
        console.log("ACCESS TOKEN VALID -> ALLOW");
    ...
    // Refresh logic will be added here later
  ...
  if (
    ...
    if (accessToken) {
      ...
      if (decoded) {
        console.log(
          "USER ALREADY AUTHENTICATED -> /explore"
        );

        return NextResponse.redirect(
          new URL("/explore", request.url)
        );
      }
    }
  }
