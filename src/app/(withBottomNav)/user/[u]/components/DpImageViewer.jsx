...
import sampleAv from "../../../../../../public/uploads/profiles/sampleAv.jpg"
...
export default function DpImageViewer({dpImage, setIsDpImageViewing, isDpImageViewing}) {
  ...
  const [liked, setLiked] = useState(currentUser?.dpLikedByMe ?? false);
  const [isDownloading, setIsDownloading] = useState(false);
const [likeLoading, setLikeLoading] = useState(false)



  const [likeCount, setLikeCount] = useState(currentUser?.dpLikeCount ?? 0);
...
//   console.log(likeCount, "lc")
  
  if (!currentUser) return null;
//   console.log(currentUser)

  // WHen clikck on dp like button 
...
// console.log(data)
  ...
  const handleDownload = async () => {
    if (!currentUser?.image || isDownloading) return;

    try {
      setIsDownloading(true);

      const response = await fetch(currentUser?.image);
      ...
      link.href = blobUrl;
      link.download = `${currentUser?.username + Date.now() || Date.now()}.jpg`;
      ...
      // Fallback
      window.open(currentUser?.image, "_blank");
    } finally {
  ...
  return (
    ...
    //   }}
      ...
      >
        ...
        width={300}
        height={300}
          src={currentUser?.image || sampleAv}
          alt={currentUser?.caption || "Post image"}
          className="
            max-h-full
            max-w-full
            select-none
