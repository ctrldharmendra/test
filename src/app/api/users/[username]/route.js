import { NextResponse } from "next/server";
import db from "../../../../lib/db";
import authenticateAccessToken from "@/lib/authenticateAccessToken";
import { getUserFromToken } from "@/lib/getUserFromToken";

// get user by username 
export async function GET(request, { params }) {
  // const auth = await authenticateAccessToken(request);

  // if (auth.error) {
  //   return NextResponse.json(
  //     { success: false, message: auth.error },
  //     { status: auth.status }
  //   );
  // }
let connection;

  try {
    const { username } = await params;

    // Logged-in user
    const loggedInUser = await getUserFromToken();
    const loggedInUserId = loggedInUser?.userId;

    if (!loggedInUserId) {
      return NextResponse.json(
        { success: false, message: "Unable to identify logged-in user" },
        { status: 401 }
      );
    }


    const { searchParams } = new URL(request.url);


    // ==========================================
    // 1. GET LOGGED-IN USER'S LOCATION
    //    (needed to compute distance to target user)
    // ==========================================

    connection = await db.getConnection();
    await connection.beginTransaction();

    const [myLocationRows] = await connection.execute(
      `
        SELECT latitude, longitude
        FROM user_locations
        WHERE user_id = ?
        LIMIT 1
      `,
      [loggedInUserId]
    );

    const myLatitude =
      myLocationRows.length > 0 ? Number(myLocationRows[0].latitude) : null;
    const myLongitude =
      myLocationRows.length > 0 ? Number(myLocationRows[0].longitude) : null;

    const hasMyLocation =
      Number.isFinite(myLatitude) && Number.isFinite(myLongitude);

    // ==========================================
    // 2. FIND TARGET USER BY USERNAME
    //    (now includes age + location fields)
    // ==========================================

    let userQuery = `
      SELECT
        u.id,
        u.username,
        u.fullname,
        u.email,
        u.gender,
        u.dob,
        u.dp,

            (
      SELECT COUNT(*)
      FROM dp_likes dl
      WHERE dl.profile_user_id = u.id
    ) AS dp_like_count,

    (
      SELECT COUNT(*)
      FROM dp_likes dl2
      WHERE dl2.profile_user_id = u.id
        AND dl2.liker_user_id = ?
    ) AS dp_liked_by_me,

        TIMESTAMPDIFF(YEAR, u.dob, CURDATE()) AS age,

        ul.latitude,
        ul.longitude,
        ul.display_name
    `;

const userQueryParams = [loggedInUserId];

if (hasMyLocation) {
  userQuery += `
    ,
    (
      6371 * ACOS(
        LEAST(
          1,
          GREATEST(
            -1,
            COS(RADIANS(?))
            * COS(RADIANS(ul.latitude))
            * COS(RADIANS(ul.longitude) - RADIANS(?))
            + SIN(RADIANS(?))
            * SIN(RADIANS(ul.latitude))
          )
        )
      )
    ) AS distance
  `;

  userQueryParams.push(myLatitude, myLongitude, myLatitude);
}
    userQuery += `
      FROM users u
      LEFT JOIN user_locations ul ON ul.user_id = u.id
      WHERE u.username = ?
      LIMIT 1
    `;
    userQueryParams.push(username);

    const [userRows] = await connection.execute(userQuery, userQueryParams);

    if (userRows.length === 0) {
    await connection.rollback();
      return NextResponse.json(
        { success: false, message: "User not found" },
        { status: 404 }
      );
    }

    const targetUser = userRows[0];
    const targetUserId = targetUser.id;

    // ==========================================
    // 3. PAGINATION
    // ==========================================


    const page = Math.max(1, Number(searchParams.get("page")) || 1);
    const limit = Math.min(50, Number(searchParams.get("limit")) || 10);
    const offset = (page - 1) * limit;

    // ==========================================
    // 4. GET USER'S POSTS (unchanged)
    // ==========================================

    const [posts] = await connection.execute(
      `
        SELECT
          p.id,
          p.user_id,
          p.caption,
          p.image_url,
          p.created_at,

          u.username,
          u.fullname,
          u.dp,

          COUNT(DISTINCT pl.id) AS like_count,

          MAX(CASE WHEN pl.user_id = ? THEN 1 ELSE 0 END) AS liked_by_me

        FROM posts p
        INNER JOIN users u ON u.id = p.user_id
        LEFT JOIN post_likes pl ON pl.post_id = p.id

        WHERE p.user_id = ?

        GROUP BY
          p.id, p.user_id, p.caption, p.image_url, p.created_at,
          u.username, u.fullname, u.dp

        ORDER BY p.created_at DESC
        LIMIT ? OFFSET ?
      `,
      [loggedInUserId, targetUserId, limit, offset]
    );

    // ==========================================
    // 5. FORMAT POSTS
    // ==========================================

    const formattedPosts = posts.map((post) => ({
      id: post.id,
      caption: post.caption,
      imageUrl: post.image_url,
      createdAt: post.created_at,
      likeCount: Number(post.like_count),
      likedByMe: Boolean(post.liked_by_me),
      user: {
        id: post.user_id,
        username: post.username,
        fullName: post.fullname,
        image: post.dp,
      },
    }));


        // WHO VIEWED MY PROFILE IMPLEMENTATION
// NEW: visit source, for profile-view notification gating
const source = (searchParams.get("source") || "").trim().toLowerCase();
// target user profile view value 
const [targetUser_is_profile_view_notify] = await connection.execute(
  `SELECT is_profile_view_notify FROM users WHERE id = ? LIMIT 1`,
  [targetUserId]
) 
const targetUserNotifyEnabled = Boolean(targetUser_is_profile_view_notify?.[0]?.is_profile_view_notify);

// loggedIn user profile view value
const [loggedInUser_is_profile_view_notify] = await connection.execute(
  `SELECT is_profile_view_notify FROM users WHERE id = ? LIMIT 1`, 
  [loggedInUserId]
)
const loggedInUserNotifyEnabled = Boolean(loggedInUser_is_profile_view_notify?.[0]?.is_profile_view_notify);

// check allowed route only 
const ALLOWED_NOTIFY_SOURCES = ["explore", "feed", "search"];
// check if self viewed then not send 
const isSelfVisit = targetUserId === loggedInUserId;
const isValidSource = ALLOWED_NOTIFY_SOURCES.includes(source);

if (!isSelfVisit && isValidSource && loggedInUserNotifyEnabled && targetUserNotifyEnabled) {
  // check if a profile_view notification already exists in the last 1 hour
const [recentNotif] = await connection.execute(
  `
    SELECT id, created_at
    FROM notifications
    WHERE recipient_id = ?
      AND actor_id = ?
      AND type = 'profile_view'
    ORDER BY created_at DESC
    LIMIT 1
    FOR UPDATE
  `,
  [targetUserId, loggedInUserId]
);

  // console.log(recentNotif, "recentNotif")
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
  const lastNotifiedAt = recentNotif.length > 0 ? new Date(recentNotif[0].created_at) : null;

  const shouldCreateNotification = !lastNotifiedAt || lastNotifiedAt < oneHourAgo;
// console.log(shouldCreateNotification, "shouldCreateNotification")
  if (shouldCreateNotification) {
   const [createdNotication] =  await connection.execute(
      `
        INSERT INTO notifications
          (recipient_id, actor_id, type, post_id, is_seen, is_read)
        VALUES (?, ?, 'profile_view', NULL, FALSE, FALSE)
      `,
      [targetUserId, loggedInUserId]
    );

      var shouldEmitSocket = createdNotication?.affectedRows > 0;
//     if(createdNotication?.affectedRows > 0){
// // actor name 
//       const [actorRows] = await connection.execute(
//         `SELECT username, dp, id FROM users WHERE id = ?`,
//         [loggedInUserId]
//       );
//       const actor = actorRows[0];
//       // notification coun 
//               const [notificationCount] = await connection.execute(
//           `SELECT COUNT(id) AS count FROM notifications WHERE recipient_id = ?  AND is_seen = FALSE`,
//           [targetUserId]
//         )
//         if (global.io) {
//           console.log("INSIDE GLOBAL>IO")
//           global.io.to(`user:${targetUserId}`).emit("notification:new",
//             {
//             type: "profile_view",
//             actor: actor, 
//             unSeenNotificationCount: notificationCount?.[0]?.count
//              }
//             );
//         }
//     }

  }
}


    // WHO VIEWED MY PROFILE IMPLEMENTATION ENDED


    await connection.commit();

    // send realtime notification to frontend to particular user 
    if(shouldEmitSocket > 0){
// actor name 
      const [actorRows] = await db.execute(
        `SELECT username, dp, id FROM users WHERE id = ?`,
        [loggedInUserId]
      );
      const actor = actorRows[0];
      // notification coun 
              const [notificationCount] = await db.execute(
          `SELECT COUNT(id) AS count FROM notifications WHERE recipient_id = ?  AND is_seen = FALSE`,
          [targetUserId]
        )
        if (global.io) {
          console.log("INSIDE GLOBAL>IO")
          global.io.to(`user:${targetUserId}`).emit("notification:new",
            {
            type: "profile_view",
            actor: actor, 
            unSeenNotificationCount: notificationCount?.[0]?.count
             }
            );
        }
    }

    // ==========================================
    // 6. RESPONSE
    // ==========================================

    return NextResponse.json(
      {
        success: true,

        user: {
          id: targetUser.id,
          username: targetUser.username,
          fullName: targetUser.fullname,
          email: targetUser.email,
          gender: targetUser.gender,
          dob: targetUser.dob,
          age: targetUser.age !== null ? Number(targetUser.age) : null,
          image: targetUser.dp,
          dpLikeCount: Number(targetUser.dp_like_count),
dpLikedByMe: Boolean(targetUser.dp_liked_by_me),
  isOnline: global.onlineUsers?.has(targetUser.id) ?? false,

          location: targetUser.display_name || null,
          latitude:
            targetUser.latitude !== null ? Number(targetUser.latitude) : null,
          longitude:
            targetUser.longitude !== null
              ? Number(targetUser.longitude)
              : null,

          distance:
            hasMyLocation && targetUser.distance !== undefined
              ? Number(Number(targetUser.distance).toFixed(1))
              : null,
        },

        page,
        count: formattedPosts.length,
        posts: formattedPosts,
      },
      { status: 200 }
    );
  } catch (error) {
        if (connection) {
      try {
        await connection.rollback();
      } catch (rollbackError) {
        console.error("Rollback error:", rollbackError);
      }
    }
    console.error("Get user posts error:", error);

    return NextResponse.json(
      { success: false, message: error.message || "Failed to load user" },
      { status: 500 }
    );
  }
  finally {
    if (connection) connection.release();
  }
}